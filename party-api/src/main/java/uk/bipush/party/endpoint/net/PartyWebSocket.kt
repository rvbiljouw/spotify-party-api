...
import uk.bipush.queue.rmq.ConsumerCallback
import uk.bipush.queue.rmq.ConsumerResult
import uk.bipush.queue.rmq.RMQConsumer
import uk.bipush.queue.rmq.RMQPublisher
import java.util.*
...
class PartyWebSocket {
    ...
    companion object {
        ...
        private val consumer = RMQConsumer<QueuedPartyMessage>(
                System.getenv("RMQ_URI"),
                "party-messages",
                "fanout",
                true,
                dataType = QueuedPartyMessage::class.java,
                callback = messageHandler)

        private val publisher: RMQPublisher<QueuedPartyMessage> = RMQPublisher<QueuedPartyMessage>(
                System.getenv("RMQ_URI"),
                "party-messages",
                "fanout",
                true)

        private val messageHandler: ConsumerCallback<QueuedPartyMessage>
            get() = object : ConsumerCallback<QueuedPartyMessage> {
                override fun handle(wrapper: QueuedPartyMessage, result: ConsumerResult) {
                    val inverse = connections.inverse()
        ...
        fun sendMessage(message: String, accounts: Set<PartyMember>) {
            publisher.publish(QueuedPartyMessage(accounts.map { it.id }.filterNotNull(), message))
        }
...
data class QueuedPartyMessage(val partyMemberIds: List<Long>, val message: String)
